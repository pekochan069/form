...
Issues and PRDs for this repo live as GitHub issues in `pekochan069/form`. Use the `gh` CLI for all operations.
...
**PRs as a request surface: yes.** External PRs are treated as incoming requests and may be processed by `/triage` using the same labels and states as issues. Collaborators' in-flight PRs are not part of this queue.

When triaging PRs, use the `gh pr` equivalents:
...
Run `gh issue view <number> --comments` for issues. For PRs, run `gh pr view <number> --comments` and inspect the diff with `gh pr diff <number>`.
