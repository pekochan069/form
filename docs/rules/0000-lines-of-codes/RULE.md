...
- This rule mainly applies to application code in `*.rs`.
- Count lines from the function declaration to its closing brace.
- Include blank lines, comments, and inline objects in the count.
- Do not treat extracted constants or helper funcions in other files as part of the same line count.
